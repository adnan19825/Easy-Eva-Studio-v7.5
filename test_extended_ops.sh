#!/bin/bash
# Automatischer Test für erweiterte FHE-Operationen
set -e

echo "=== AUTOMATISIERTER TEST: ERWEITERTE FHE-OPS ==="
cd ~/fhe-workspace

# 1. Kompilieren
echo "[1/4] Kompiliere fhe-core mit neuen Operationen..."
cargo build --release -p fhe-core

# 2. Testprogramm erstellen
echo "[2/4] Erstelle Testprogramm..."
cat > test_phase1.rs << 'RUSTCODE'
use fhe_core::*;
use std::error::Error;
...
# 3. Test kompilieren und ausführen
echo "[3/4] Kompiliere und führe Tests aus..."
rustc --edition=2021 \
  -L ./target/release/deps \
  --extern fhe_core=./target/release/libfhe_core.rlib \
  test_phase1.rs

echo "[4/4] Führe Tests aus..."
./test_phase1

# 4. Aufräumen
rm -f test_phase1 test_phase1.rs
echo "\\n✅ Automatisierter Test abgeschlossen!"
